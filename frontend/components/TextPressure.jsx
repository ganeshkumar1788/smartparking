...
const TextPressure = ({
    ...
    // This font is just an example. Do not use commercially.
    ...
    const setSize = useCallback(() => {
        ...
        const availW = containerW - 16; // 8px padding on each side

        // Start with an estimate
        let newFontSize = availW / (chars.length / 1.6);
        ...
        // After the browser renders, check actual text width and shrink if needed
        requestAnimationFrame(() => {
            if (!titleRef.current) return;
            let currentSize = newFontSize;
            // Shrink until the rendered h1 fits within available width
            while (
                titleRef.current.scrollWidth > availW &&
                currentSize > minFontSize
            ) {
                currentSize = Math.max(currentSize * 0.92, minFontSize);
